...
const mongoURI = "mongodb://id:pw@localhost:27017/" + dbName + "?authMechanism=SCRAM-SHA-256&authSource=admin"
...
func main() {
	...
	app.Get("/", func(c *fiber.Ctx) error {
		...
		return errors.New("Hello, World! Failed")
	})
	...
	app.Put("/employee/:id", func(c *fiber.Ctx) error {
		...
		if err != nil {
			if err == mongo.ErrNoDocuments {
				return c.Status(http.StatusNotFound).SendString("Employee not found")
			}
			return c.Status(500).SendString(err.Error())
		}
	...
	//defer mg.Client.Disconnect(ctx)
